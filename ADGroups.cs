    ...
    class MainClass {
        ...
        private static void logUsersToArcsight(List<AGroup> aDGroupUsers) {
            ...
            foreach (AGroup group in aDGroupUsers) {
                foreach (User user in group.members) {
                    ...
                    var message = 
                        "CEF:0|PigeonAdLogger|PigeonAdLogger|1.0|" +
                        appSignature + "|ActiveDirectoryGroupUsers|Mid|act=%<dispositionString> dvc=%<sourceServer> dst=%<destination> dhost=%<urlHost> dpt=%<port> src=%<source> spt=%<clientSourcePort> suser=" +
                        user.sicil + " destinationTranslatedPort=%<proxySourcePort> rt=%<time>000 in=%<bytesSent> out=%<bytesReceived> requestMethod=%<method> requestClientApplication=%<=userAgent> reason=%<scanReasonString> cs1Label=user cs1=" +
                        user.sicil + " cs2Label=ADGroup cs2=" +
                        group.name + " cs3Label=FullName cs3=" +
                        user.fullname + " cs4Label=Mobile cs4=" +
                        user.mobile.Replace("+90", "")  +
                        " cn1=%<=dispositionNumber> cn2=%<scanDuration> request=%<=url>";
                    logger.Info(message);
                    string toFile = String.Join(",", megaCount-1, DateTime.Now.ToString(), appSignature, user.sicil, group.name, user.fullname, user.mobile);
                    // print to console
                    Console.WriteFormatted("\n" + toFile,Color.Green);
                    // print to file
        ...
        static public List<AGroup> getGroupUsers3(List<string> ADgroups) {
            ...
            foreach (string currGroup in ADgroups) {
                ...
                //var dsad = GetUsersInGroup("int.teb.com.tr", currGroup);
        ...
        private static List<string> GetGroupMembersName(string groupName) {
            ...
            //new DirectoryEntry("LDAP://CN=" + groupName + ",OU=Exchange Dist. Grup,OU=Exchange,DC=int,DC=teb,DC=com,DC=tr");
            ...
            while (true) {
                ...
                foreach (var member in memberDns) {
                    try {
                        /*
                        "CN=Enterprise Admins,OU=Exchange Dist. Grup,OU=Exchange,DC=int,DC=teb,DC=com,DC=tr"
                        ...
                        */
                        ...
                        if (temp.Contains('=')) {
                            //int.teb.com.tr/DelegatedObjects/3LevelDelegates/Users/dm_p5775
                ...
                if (memberDns.Count == 0 || (memberDns.Value.ToString().Contains("CM_MO") == true && memberDns.Count == 1)) break;
                try {
